// Function to reset token and clear data
const resetToken = async (): Promise<void> => {
  try {
    await SecureStore.deleteItemAsync('ouraToken');
    setOuraToken('');
    setTokenSaved(false);
    setHeartRate(null);
    setCurrentZone(null);
    ...
    openSetupDialog();
    Alert.alert('Token Removed', 'Your Oura token has been removed. Please enter a new token to continue using the app.');
  } catch (err) {
    console.error('Error resetting token:', err);
    Alert.alert('Error', 'Failed to reset your token. Please try again.');
  }
...
const saveToken = async (): Promise<void> => {
  ...
  try {
    ...
    if (!isValid) {
      Alert.alert(
        'Invalid Token', 
        'The provided token could not be validated with Oura API. Please check that you have entered the correct token and that it has the necessary permissions (heartrate:read).',
        [
          { 
            text: 'Get Help', 
            onPress: () => Linking.openURL('https://cloud.ouraring.com/docs/authentication')
          },
          { text: 'OK' }
        ]
      );
...
const fetchLatestHeartRate = async (): Promise<void> => {
  ...
  try {
    ...
    if (!isMonitoring) {
      if (axios.isAxiosError(err) && err.response) {
        if (err.response.status === 401) {
          setError('Authentication failed. Your token may be invalid or expired. Please try setting up your token again.');
        } else if (err.response.status === 403) {
          setError('Your token lacks necessary permissions. Make sure it has heartrate:read permission.');
        } else {
...
const fetchHistoricalData = async (start: Date, end: Date): Promise<void> => {
  ...
  try {
    ...
    if (axios.isAxiosError(err) && err.response) {
      ...
      setError('Request timed out. The selected time range may be too large or your internet connection may be slow.');
    } else {
      setError(`Failed to fetch heart rate data: ${err.message}`);
    }
    ...
    // If token seems invalid, prompt user to update it
    if (axios.isAxiosError(err) && err.response?.status === 401) {
      setTimeout(() => {
        Alert.alert(
          'Authentication Problem',
          'Your Oura token appears to be invalid or expired. Would you like to update it now?',
          [
            {
              text: 'Update Token',
  ...
  <PaperProvider>
    <View style={styles.container}>
      <Appbar.Header style={styles.header}>
        ...
        {tokenSaved && <Appbar.Action icon="refresh" onPress={() => fetchHistoricalData(startDate, endDate)} />}
        ...
        >
          ...
          <Dialog.Content>
            <Text style={styles.dialogText}>
              ...
              You can get this token from the <Text 
          ...
          <Dialog.Actions>
            ...
            {tokenSaved && (
              <Button onPress={resetToken} color="#B56C6C">Reset</Button>
            )}
